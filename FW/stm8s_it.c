...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
extern volatile bool PIR_Motion, RADAR_Motion;
...
INTERRUPT_HANDLER(EXTI_PORTC_IRQHandler, 5)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
  if (GPIO_ReadInputPin(RADAR_GPIO_PORT, (GPIO_Pin_TypeDef)RADAR_PIN))
  {
    RADAR_Motion = TRUE;
    if (Status_Disp_EN) GPIO_WriteHigh(STATUS_2_GPIO_PORT, (GPIO_Pin_TypeDef) STATUS_2_PIN);
    RADAR_Conf_Timer = RADAR_DELAY_MS;
  } else
  {
    RADAR_Motion = FALSE;
  ...
  if (GPIO_ReadInputPin(PIR_GPIO_PORT, (GPIO_Pin_TypeDef) PIR_PIN))
  {
    PIR_Motion = TRUE;
    if (Status_Disp_EN) GPIO_WriteHigh(STATUS_3_GPIO_PORT, (GPIO_Pin_TypeDef) STATUS_3_PIN);
    PIR_Conf_Timer = PIR_DELAY_MS;
  } else
  {
    PIR_Motion = FALSE;
